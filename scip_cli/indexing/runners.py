...
def run_indexer_command(binary, args, cwd, env):
    """Try to run an indexer binary. Returns (success, result)."""
    try:
        result = run_subprocess([binary, *args], cwd, env=env)
...
def install_via_go_install(package, binary, args, cwd, env, *, version: str | None = None):
    ...
    if go_binary.exists():
        ...
        return run_subprocess([str(go_binary), *args], cwd, env=go_env)
...
def install_via_rustup(component, binary, args, cwd, env):
    ...
    return run_subprocess([binary, *args], cwd, env=env)
...
def run_indexer_with_fallback(
    ...
    success, result = run_indexer_command(binary, args, cwd, run_env)
    if success:
        assert result is not None
        return result

    if go_package:
        return install_via_go_install(go_package, binary, args, cwd, run_env, version=go_version)
    if rustup_component:
