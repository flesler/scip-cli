...
def _make_db():
    ...
    conn.execute("INSERT INTO defn_enclosing_ranges VALUES (1, 1, 10, 12)")
    conn.execute("INSERT INTO defn_enclosing_ranges VALUES (2, 2, 4, 6)")
    conn.commit()
    return conn
...
class TestMachineOutputFlags:
    def test_search_names_only(self, capsys):
        ...
        with (
            patch("scip_cli.commands.search.setup", return_value=(db, Path("/proj"))),
            patch("scip_cli.commands.search.path_scope_from_args", return_value=None),
        ):
            search.main(args)
    ...
    def test_refs_paths_only(self, capsys):
        ...
        with (
            patch("scip_cli.commands.refs.setup", return_value=(db, project_root)),
    ...
    def test_members_names_only(self, capsys):
        ...
        db.execute("UPDATE global_symbols SET symbol = 'parent#' WHERE id = 1")
        db.commit()

        args = SimpleNamespace(symbol="MyClass", limit=10, path=None, names_only=True)
        with (
            patch("scip_cli.commands.members.setup", return_value=(db, Path("/proj"))),
            patch("scip_cli.commands.members.path_scope_from_args", return_value=None),
            patch(
